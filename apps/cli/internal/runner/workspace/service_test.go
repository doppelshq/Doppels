...
func TestServiceDuplicateCapabilityRevisionIsInvalidAndAmbiguous(t *testing.T) {
	service, base := newService(t)
	root := initRoot(t, filepath.Join(base, "space-a"))
...
`)
	...
	if summary.Health != "invalidManifests" || summary.Capabilities != 2 {
		t.Fatalf("summary = %+v, want duplicate manifests counted and unhealthy", summary)
	}
	if _, err := service.GetCapability(root, "cap-a", "1.0.0"); !errors.Is(err, ErrCapabilityAmbiguous) {
		t.Fatalf("GetCapability duplicate revision error = %v, want ambiguity", err)
	}
}
