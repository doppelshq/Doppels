...
func (app *App) runShare(arguments []string) int {
	...
	request, alreadyComplete, err := listener.AwaitSharedRequest(waitContext, client, channel, created, initial)
	waitSpin.Stop()
	cancelWait()
	...
	if expiredBeforeRequest {
		...
		cancelRecovery()
		if pendingErr == nil {
			recoveredRequest, recoveredComplete, recoveryErr := listener.RequestFromPending(created, pending)
			switch {
